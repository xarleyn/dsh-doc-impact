...
// lib/client.js — dsh-doc-impact browser bundle (hand-written CJS factory,
// loaded by the dsh web ModuleLoader and served at /plugins/dsh-doc-impact/client.js).
//
    ...
    SettingsForm.prototype.plan = function () {
      ...
      this.staged.forEach(function (staged, field) {
        ...
        if (spec.kind === "text" || spec.kind === "number") {
          if (staged.text === (spec.kind === "number" ? formatNumber(self.sectionValue(field)) : formatText(self.sectionValue(field)))) return;
          ...
          if (write === undefined) {
            ...
            plan.push({
              field: field,
              run: function () {
                return self.runSet(field, write.value);
              }
            });
          }
          return;
        ...
        if (staged.value === self.sectionValue(field)) return;
        plan.push({
          field: field,
          run: function () {
            return self.runSet(field, staged.value);
          }
        });
      });
      return plan;
    ...
    SettingsForm.prototype.actions = function () {
      ...
      return {
        edit: function (field, text) {
          self.stage(field, { op: "set", text: text });
        },
        choose: function (field, value) {
          self.stage(field, { op: "set", value: value });
        },
        resetField: function (field) {
          var spec = specOf(field);
          if (spec.kind === "text" || spec.kind === "number") {
            var cleared = self.clearedValue(field);
            self.stage(field, {
              op: "set",
              text: spec.kind === "number" ? formatNumber(cleared) : formatText(cleared),
              clear: true
